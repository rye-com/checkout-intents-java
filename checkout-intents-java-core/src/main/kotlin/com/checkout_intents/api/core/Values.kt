...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { CheckoutIntentsInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw CheckoutIntentsInvalidDataException("`$name` is not set")
            is JsonNull -> throw CheckoutIntentsInvalidDataException("`$name` is null")
            else -> throw CheckoutIntentsInvalidDataException("`$name` is invalid, received $this")
        }
